...
export default function Torneos() {
    ...
    axios
      ...
      .then((response) => {
        const data = response.data.filter(
          (item) => item.UsuarioCreador === Number(localStorage.userID)
        );
        getTorneosUser(data);
      })
  ...
  function confirmacion() {
    if (window.confirm("¿Realmente queres borrar el torneo?")) {
      ...
      axios
        .delete(
          `http://127.0.0.1:3001/api/torneos/${torneosDelUser[0].TorneoID}`
        )
        .then((response) => {
          console.log("hecho");
        })
  ...
  const unirmeTorneo = (e) => {
    ...
    axios
      ...
      .then((response) => {
        ...
        Swal.fire({
          title: "Excelente!",
          text: "Te uniste al torneo exitosamente",
          icon: "success",
          timer: 2000,
        });
        setTimeout(() => {
  ...
  if (torneosDelUser.length === 1) {
    axios
      .get(
        `http://127.0.0.1:3001/api/participantesxtorneo/${torneosDelUser[0].TorneoID}`
      )
      .then((response) => {
