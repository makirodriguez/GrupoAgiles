...
app.get("/api/partidos", (req, res) => {
  const sql =
    "SELECT PartidoID, UTCDATE, GolesLocal, GolesVisit, Score, Equipo.Nombre Local, Equipo.ImgPath LocalPath, a.Nombre Visitante, a.ImgPath VisitantePath FROM Partido inner join Equipo on Equipo.EquipoID = Partido.LocalID inner join Equipo a on a.EquipoID = Partido.VisitanteID ORDER BY UTCDATE";
  db.all(sql, [], (err, rows) => {
    if (err) {
      return console.error(err.message);
    }
...
app.get("/api/prediccionesPorPartido", (req, res) => {
  const sql =
    "SELECT Partido.PartidoID, UTCDATE, GolesLocal, GolesVisit, Score, Equipo.Nombre Local, Equipo.ImgPath LocalPath, a.Nombre Visitante, a.ImgPath VisitantePath, Usuario.Nombre, Prediccion.Resultado FROM Partido inner join Equipo on Equipo.EquipoID = Partido.LocalID  inner join Equipo a on a.EquipoID = Partido.VisitanteID  inner join Prediccion on Partido.PartidoID=Prediccion.PartidoID inner join Usuario on Prediccion.UsuarioID=Usuario.UsuarioID  ORDER BY Partido.PartidoID ";
  db.all(sql, [], (err, rows) => {
    if (err) {
      return console.error(err.message);
    }
...
app.get("/api/predicciones", (req, res) => {
  const sql =
    "SELECT PrediccionID, Resultado, Usuario.Nombre, Partido.PartidoID, Partido.UTCDATE FROM Prediccion inner join Usuario on Prediccion.UsuarioID = Usuario.UsuarioID inner join Partido on Prediccion.PartidoID = Partido.PartidoID ORDER BY Partido.PartidoID";
  db.all(sql, [], (err, rows) => {
    if (err) {
      return console.error(err.message);
    }
...
app.get("/api/partidos/:id", (req, res) => {
  ...
  const sql =
    "SELECT PartidoID, UTCDATE, GolesLocal, GolesVisit, Score, Equipo.Nombre Local, Equipo.ImgPath LocalPath, a.Nombre Visitante, a.ImgPath VisitantePath FROM Partido inner join Equipo on Equipo.EquipoID = Partido.LocalID inner join Equipo a on a.EquipoID = Partido.VisitanteID WHERE PartidoID = ?";
...
app.get("/api/predicciones/:id", (req, res) => {
  const id = req.params.id;
  const sql =
    "SELECT PrediccionID, Resultado, Usuario.Nombre, Partido.PartidoID, Partido.UTCDATE FROM Prediccion inner join Usuario on Prediccion.UsuarioID = Usuario.UsuarioID inner join Partido on Prediccion.PartidoID = Partido.PartidoID WHERE PrediccionID = ?";
  db.all(sql, id, (err, rows) => {
    if (err) {
      return console.error(err.message);
    }
...
app.post("/api/partidos", (req, res) => {
  const sql =
    "INSERT INTO Partido (GolesLocal, GolesVisit, UTCDATE, Score, LocalID, VisitanteID) VALUES (?, ?, ?, ?, ?, ?)";
  const partido = [
    req.body.GolesLocal,
    req.body.GolesVisit,
    req.body.Utcdate,
...
app.post("/api/predicciones", (req, res) => {
  const sql =
    "INSERT INTO Prediccion (Resultado, PartidoID, UsuarioID) VALUES (?, ?, ?)";
...
app.put("/api/partidos/:id", (req, res) => {
  ...
  const sql =
    "UPDATE Partido SET GolesLocal = ?, GolesVisit = ?, UTCDATE = ?, Score = ?, LocalID = ?, VisitanteID = ? WHERE (PartidoID = ?)";
...
app.put("/api/predicciones/:id", (req, res) => {
  ...
  const sql =
    "UPDATE Equipo SET Resultado = ?, PartidoID = ?, UsuarioID = ? WHERE (PrediccionID = ?)";
...
axios
  ...
  .then((res) => {
    res.data.matches.map((x) => {
      const sql =
        "INSERT or REPLACE INTO Partido (PartidoID, GolesLocal, GolesVisit, UTCDATE, Score, LocalID, VisitanteID) VALUES (?, ?, ?, ?, ?, ?, ?)";
      const partido = [
        x.id,
